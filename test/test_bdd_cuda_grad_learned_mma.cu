...
mgxthrust::device_vector<double> compute_expected_mm_diff(const char* instance)
{
    ...
    mgxthrust::device_vector<double> expected_mm_diff(solver.nr_layers());
    const auto mms = solver.min_marginals_cuda(false);
    const auto& mms_0 = std::get<1>(mms);
    const auto& mms_1 = std::get<2>(mms);
    mgxthrust::transform(mms_1.begin(), mms_1.end(), mms_0.begin(), expected_mm_diff.begin(), mgxthrust::minus<double>());
...
{
    const mgxthrust::device_vector<double> expected_mm_diff = compute_expected_mm_diff(instance);
    ILP_input ilp = ILP_parser::parse_string(instance);
    bdd_preprocessor bdd_pre(ilp);
    bdd_collection bdd_col = bdd_pre.get_bdd_collection();
    ...
    for(int learning_itr = 0; learning_itr < num_learning_itr; learning_itr++)
    {
        ...
        const mgxthrust::device_vector<int> pv = solver.get_primal_variable_index();
        loss_func compute_loss({mgxthrust::raw_pointer_cast(pv.data()),
        ...
        //Backward pass:
        loss_gradient_func compute_loss_grad({mgxthrust::raw_pointer_cast(solver.get_primal_variable_index().data()),
                                            mgxthrust::raw_pointer_cast(final_mm_diff.data()),
                                            mgxthrust::raw_pointer_cast(expected_mm_diff.data()),
        ...
        // mgxthrust::device_vector<double> pert_lo_costs(std::get<0>(final_costs));
        // mgxthrust::device_vector<double> pert_hi_costs(std::get<1>(final_costs));
        // grad_step_min_marginals grad_step_mm_func({
        //                                         mgxthrust::raw_pointer_cast(grad_lo_costs.data()),
        //                                         mgxthrust::raw_pointer_cast(grad_hi_costs.data()),
        //                                         mgxthrust::raw_pointer_cast(pert_lo_costs.data()),
        //                                         mgxthrust::raw_pointer_cast(pert_hi_costs.data()),
        //                                         5e-3});
        // mgxthrust::for_each(mgxthrust::make_counting_iterator<int>(0), mgxthrust::make_counting_iterator<int>(0) + solver.nr_layers(), grad_step_mm_func);
        ...
        // mgxthrust::transform(pmms_1.begin(), pmms_1.end(), pmms_0.begin(), final_mm_diff.begin(), mgxthrust::minus<double>());
        // mgxthrust::device_vector<double> lossp(final_mm_diff.size());
        // loss_func compute_lossp({mgxthrust::raw_pointer_cast(solver.get_primal_variable_index().data()),
        //                         mgxthrust::raw_pointer_cast(final_mm_diff.data()),
        //                         mgxthrust::raw_pointer_cast(expected_mm_diff.data()),
        //                         mgxthrust::raw_pointer_cast(lossp.data()),
        //                         solver.nr_variables()});

        // mgxthrust::for_each(mgxthrust::make_counting_iterator<int>(0), mgxthrust::make_counting_iterator<int>(0) + solver.nr_layers(), compute_lossp);
        // const double loss_valp = mgxthrust::reduce(lossp.begin(), lossp.end());
        ...
        mgxthrust::device_vector<double> grad_omega(1);
        ...
        grad_step_dist_w grad_step_func({
            mgxthrust::raw_pointer_cast(grad_dist_weights.data()),
            mgxthrust::raw_pointer_cast(dist_weights.data()),
            2.5e-4});
        mgxthrust::for_each(mgxthrust::make_counting_iterator<int>(0), mgxthrust::make_counting_iterator<int>(0) + solver.nr_layers(), grad_step_func);
    }
