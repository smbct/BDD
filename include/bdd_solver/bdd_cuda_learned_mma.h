...
namespace LPMP {
    ...
    class bdd_cuda_learned_mma : public bdd_cuda_parallel_mma<REAL> {
        public:
            ...
            int iterations(const mgxthrust::device_ptr<const REAL> dist_weights, 
                            const int num_itr, 
                            const REAL omega_scalar, // will not be used if omega_vec != nullptr
                            const double improvement_slope = 1e-6,
                            mgxthrust::device_ptr<REAL> sol_avg = nullptr,
                            mgxthrust::device_ptr<REAL> lb_first_diff_avg = nullptr,
                            mgxthrust::device_ptr<REAL> lb_second_diff_avg = nullptr,
                            const int compute_history_for_itr = 0,
                            const REAL history_avg_beta = 0.9,
                            const mgxthrust::device_ptr<const REAL> omega_vec = nullptr);
            ...
            void grad_iterations(
                ...
                mgxthrust::device_ptr<REAL> grad_dist_weights_out, // Output: contains grad w.r.t distribution weights, assumes the memory is already allocated (= nr_layers()).
                mgxthrust::device_ptr<REAL> grad_omega,    // Output: contains grad w.r.t omega. Should be of size 1 if omega_vec is nullptr otherwise size of omega_vec.
                const REAL omega_scalar, // will not be used if omega_vec != nullptr
        ...
        private:
            ...
            void forward_iteration_learned_mm_dist(const mgxthrust::device_ptr<const REAL> dist_weights, mgxthrust::device_ptr<REAL> mm_diff_ptr, const REAL omega_scalar, const mgxthrust::device_ptr<const REAL> omega_vec = nullptr);

            // similar to above except that last_hop should be equal to 0 for complete backward pass.
            void backward_iteration_learned_mm_dist(const mgxthrust::device_ptr<const REAL> dist_weights, mgxthrust::device_ptr<REAL> mm_diff_ptr, const REAL omega_scalar, const mgxthrust::device_ptr<const REAL> omega_vec = nullptr); 
            ...
            void grad_forward_iteration_learned_mm_dist(
                mgxthrust::device_ptr<REAL> deferred_min_marg_diff, // deferred min-marginals used in forward pass, output will contains deferred min-marginals after forward iteration (not useful)
                const mgxthrust::device_ptr<const REAL> dist_weights, // distribution weights used in the forward pass.
                ...
                mgxthrust::device_ptr<REAL> grad_dist_weights_out, // Output: contains grad w.r.t distribution weights, assumes the memory is already allocated (= nr_layers()).
                const REAL omega_scalar, // will not be used if omega_vec != nullptr
                mgxthrust::device_ptr<REAL> grad_omega,    // Output: contains grad w.r.t omega (size = 1) if omega_vec = nullptr otherwise size should be same size of omega_vec.
                const mgxthrust::device_ptr<const REAL> omega_vec = nullptr
                );
            ...
            void grad_backward_iteration_learned_mm_dist(
                ...
                mgxthrust::device_ptr<REAL> grad_hi_cost, // Input: incoming grad w.r.t hi_cost current iteration and Outputs in-place to compute grad. hi_cost before iteration.
                mgxthrust::device_ptr<REAL> grad_cost_from_root, // Input: incoming grad w.r.t cost_from_root (size = nr_bdd_nodes()), Outputs in-place to compute grad before iteration.
                mgxthrust::device_ptr<REAL> grad_cost_from_terminal, // Input: incoming grad w.r.t cost_from_terminal (size = nr_bdd_nodes()) iteration and Outputs in-place to compute grad before iteration.
                mgxthrust::device_ptr<REAL> grad_mm, // Input: incoming grad w.r.t min-marg. diff. current iteration and Outputs in-place to compute grad. w.r.t deferred min-marginals used in iteration.
                mgxthrust::device_ptr<REAL> grad_dist_weights_out, // Output: contains grad w.r.t distribution weights, assumes the memory is already allocated (= nr_layers()).
                const REAL omega_scalar, // will not be used if omega_vec != nullptr
                mgxthrust::device_ptr<REAL> grad_omega,    // Output: contains grad w.r.t omega (size = 1) if omega_vec = nullptr otherwise size should be same size of omega_vec.
            ...
            void grad_hop_update_learned_mm_dist(
                ...
                mgxthrust::device_ptr<REAL> grad_mm,     // Input: incoming grad w.r.t min-marginal differences current hop update. Is overwritten by accumulated grad_mm (not useful).
                mgxthrust::device_ptr<REAL> grad_dist_weights_out, // Output: contains grad w.r.t distribution weights, assumes the memory is already allocated (= nr_layers())
            ...
            void compute_grad_cost_from_root(mgxthrust::device_ptr<REAL> grad_cost_from_root,  // incoming gradient of hop_index + 1 root costs is used to compute grad for hop_index root costs.
            ...
            void compute_grad_cost_from_terminal(mgxthrust::device_ptr<REAL> grad_cost_from_terminal,  // incoming gradient of hop_index terminal costs is used to compute grad for hop_index + 1 terminal costs.
                                                mgxthrust::device_ptr<REAL> grad_lo_cost,          // accumulates gradient for hop_index
                                                mgxthrust::device_ptr<REAL> grad_hi_cost,          // accumulates gradient for hop_index
                                                const int hop_index);

            void grad_mm_diff_of_hop(const mgxthrust::device_ptr<const REAL> before_update_lo_cost, // min-marginal computation was done on input costs (not updated) costs.
                                    const mgxthrust::device_ptr<const REAL> before_update_hi_cost,
                                    const mgxthrust::device_ptr<const REAL> cur_min_marg_diff, // current min-marginals which were subtracted in present iteration.
                                    mgxthrust::device_ptr<REAL> incoming_grad_mm_diff_hop, // gradient of min-marginal diff. output is backpropagates through multiplication by omega .
