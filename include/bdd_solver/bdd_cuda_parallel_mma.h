...
namespace LPMP {
    ...
    class bdd_cuda_parallel_mma : public bdd_cuda_base<REAL> {
        public:
            ...
            void forward_mm(const REAL omega, mgxthrust::device_vector<REAL>& delta_lo_hi);
            void backward_mm(const REAL omega, mgxthrust::device_vector<REAL>& delta_lo_hi);

            // Normalize delta by num BDDs to distribute isotropically.
            // delta_lo_ -> delta_hi_/#BDDs, delta_hi_ -> delta_hi_/#BDDs
            void normalize_delta(mgxthrust::device_vector<REAL>& delta_lo_hi) const;

            mgxthrust::device_vector<REAL> net_solver_costs() const;

            void gradient_step(const mgxthrust::device_vector<REAL>& g, double step_size);
        ...
        protected:
            ...
            void min_marginals_from_directional_costs(const int hop_index, const REAL omega_scalar, mgxthrust::device_ptr<REAL> mm_diff_ptr, const mgxthrust::device_ptr<const REAL> omega_vec = nullptr);

            // compute delta_lo_ and delta_hi_ (per variable) from mm_to_distribute (per bdd node)
            void compute_delta(const mgxthrust::device_ptr<const REAL> mm_to_distribute, mgxthrust::device_ptr<REAL> delta_lo_hi) const;

            // set argument to all infinity
            void flush_mm(mgxthrust::device_ptr<REAL> mm_diff_ptr);

            mgxthrust::device_vector<REAL> hi_cost_out_, lo_cost_out_; // One entry per BDD layer.
        ...
        private:
            ...
            mgxthrust::device_vector<REAL> mm_lo_local_; // Contains mm_lo for last computed hop. Memory allocated is as per max(cum_nr_layers_per_hop_dist_).
    };
    ...
    template<typename REAL>
    void bdd_cuda_parallel_mma<REAL>::gradient_step(const mgxthrust::device_vector<REAL>& g, double step_size)
    {
    ...
    void bdd_cuda_parallel_mma<REAL>::gradient_step(ITERATOR grad_begin, ITERATOR grad_end, double step_size)
    {
        ...
        mgxthrust::transform(this->hi_cost_.begin(), this->hi_cost_.end(), grad_begin, this->hi_cost_.begin(), 
